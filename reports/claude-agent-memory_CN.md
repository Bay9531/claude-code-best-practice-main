...
**Claude Code v2.1.33**（2026 年 2 月）引入了 `memory` frontmatter 字段，为每个子代理提供其自己的持久化基于 Markdown 的知识存储。在此之前，每次代理调用都从零开始。
...
你是一名代码审查员。在审查代码时，用你发现的模式、约定和重复问题更新你的代理记忆。
```
...
| `local` | `.claude/agent-memory-local/<agent-name>/` | 否（已被 git 忽略） | 否 | 个人的项目特定知识 |
...
2. **工具访问**：`Read`、`Write`、`Edit` 被自动启用，以便代理管理其记忆
3. **执行期间**：代理自由读写其记忆目录
4. **整理**：如果 `MEMORY.md` 超过 200 行，代理会将详细信息移至特定主题的文件中
...
这些系统是**互补的**——代理既读取 CLAUDE.md（项目上下文），也读取自己的记忆（代理特定知识）。
...
- **提示记忆的使用**——加入明确指令：`"开始前，回顾你的记忆。完成后，用你学到的内容更新你的记忆。"`
- **请求记忆检查**——在调用代理时：`"审查此 PR，并检查你的记忆中是否有你之前见过的模式。"`
