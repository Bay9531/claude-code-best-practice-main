...
API 级别的功能（现已全面上市），可减少令牌消耗、降低延迟并提高工具准确性。随 Opus/Sonnet 4.6 发布。
...
1. [概述](#overview)
2. [程序化工具调用 (PTC)](#programmatic-tool-calling-ptc)
3. [网页搜索/抓取的动态过滤](#dynamic-filtering-for-web-searchfetch)
4. [工具搜索工具](#tool-search-tool)
5. [工具使用示例](#tool-use-examples)
6. [Claude Code 相关性](#claude-code-relevance)
...
| 功能 | 解决的问题 | 令牌节省 | 可用性 |
|---------|---------------|---------------|--------------|
| 程序化工具调用 | 多步骤代理循环在往返过程中消耗令牌 | ~37% 减少 | API, Foundry (GA) |
| 动态过滤 | 网页搜索/抓取结果用无关内容膨胀上下文 | ~24% 更少输入令牌 | API, Foundry (GA) |
| 工具搜索工具 | 过多工具定义膨胀上下文 | ~85% 减少 | API, Foundry (GA) |
| 工具使用示例 | 仅凭架构无法表达使用模式 | 72% → 90% 准确率 | API, Foundry (GA) |

所有功能自 2026 年 2 月 18 日起**全面上市**。
...
**之前（传统工具调用）：**
```
用户提示词 → Claude → 工具调用 1 → 响应 1 → Claude → 工具调用 2 → 响应 2 → Claude → 工具调用 3 → 响应 3 → Claude → 最终答案
```
每次工具调用都需要一次完整的模型往返。3 个工具 = 3 次推理过程。

**之后（程序化工具调用）：**
```
用户提示词 → Claude → 编写 Python 脚本 → 脚本内部调用工具 1、工具 2、工具 3 → 标准输出 → Claude → 最终答案
```
Claude 编写编排所有工具的代码。只有最终的 `stdout` 进入上下文窗口。3 个工具 = 1 次推理过程。
...
4. 提供工具结果 — 结果进入**正在运行的代码**，而非 Claude 的上下文
5. 代码恢复执行，处理结果，必要时继续调用更多工具
...
**建议：** 每个工具选择一种模式，不要同时使用两种。这能为 Claude 提供更清晰的指导。

### 响应中的 `caller` 字段

每个工具使用块都包含一个 `caller` 字段，以便了解工具的调用方式：
...
| **无 MCP 工具** | MCP 连接器工具无法以编程方式调用 |
| **无网页搜索/抓取** | 网页工具不支持 PTC |
| **无结构化输出** | 不兼容 `strict: true` 工具 |
| **无强制工具选择** | `tool_choice` 无法强制使用 PTC |
| **容器生命周期** | 约 4.5 分钟后过期 |
| **ZDR** | 不包含在零数据保留策略中 |
...
| 好的使用场景 | 不太理想的情况 |
|----------------|------------|
| 处理需要聚合的大型数据集 | 返回简单响应的单次工具调用 |
| 3 个以上按顺序依赖的工具调用 | 需要即时用户反馈的工具 |
| 在 Claude 看到结果之前进行过滤/转换 | 非常快速的操作（开销 > 收益） |
| 跨多个项目的并行操作 | |
| 基于中间结果的条件逻辑 | |
...
- 中间处理在代码中完成，而非模型令牌
- 10 次程序化工具调用 ≈ 10 次直接调用所需令牌数的 1/10
...
网页搜索和抓取工具将完整的 HTML 页面转储到 Claude 的上下文窗口中。其中大部分内容无关紧要 — 导航、广告、样板文本。Claude 随后对所有内容进行推理，浪费令牌并降低准确性。

### 解决方案

Claude 现在**编写并执行 Python 代码来过滤网页结果**，然后再将其送入上下文窗口。Claude 不再对原始 HTML 进行推理，而是在沙箱中过滤、解析和提取仅相关的内容。
...
**默认启用** — 在使用 Sonnet 4.6 和 Opus 4.6 的新工具类型版本时默认开启。
...
**令牌效率：** 平均减少 24% 的输入令牌。Sonnet 4.6 可降低使用成本；Opus 4.6 可能因更复杂的过滤代码而略微增加。
...
提前加载所有工具定义会浪费上下文。如果你有 50 个 MCP 工具，每个约 1.5K 令牌，那就是 75K 令牌，用户甚至还没提出问题。
...
Claude Code 具有 **MCP 工具搜索自动模式**（自 v2.1.7 起默认启用）。当 MCP 工具描述超过上下文的 10% 时，它们会被延迟并通过 `MCPSearch` 发现。使用 `ENABLE_TOOL_SEARCH=auto:N` 配置阈值，其中 N 是上下文百分比（0-100）。
...
在工具定义中添加 `input_examples` — 超越模式的具體使用模式。
...
- 展示**多样性**：最小规格、部分规格和完整规格
- 保持简洁：**每个工具 1-5 个示例**
- 专注于消除歧义 — 以行为清晰度为目标，而非模式完整性
...
|---------|-------------------|--------|
| 工具搜索 | 自 v2.1.7 起内建为 MCPSearch 自动模式 | 如果你有很多 MCP 工具，调整 `ENABLE_TOOL_SEARCH=auto:N` |
| 动态过滤 | CLI 中不可用（API 级别的网页工具） | 适用于进行网页研究的 Agent SDK 用户 |
...
如果你正在构建自定义 MCP 服务器，工具使用示例可以改进 Claude 使用你工具的方式：
