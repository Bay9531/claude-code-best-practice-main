...
Claude Code 加载动画下旋转显示的词语和提示。提取自 `~/.local/share/claude/versions/2.1.121`。
...
| ID | 提示内容 |
|---|---|
| new-user-warmup | 从小的功能或错误修复开始，让 Claude 提出方案，并验证其建议的编辑 |
| default-permission-mode-config | 使用 /config 更改默认权限模式（包含计划模式） |
| git-worktrees | 使用 git worktrees 同时运行多个 Claude 会话 |
...
| colorterm-truecolor | 尝试设置环境变量 COLORTERM=truecolor 以获得更丰富的色彩 |
| powershell-tool-env | 设置 CLAUDE_CODE_USE_POWERSHELL_TOOL=1 以启用 PowerShell 工具（预览版） |
| status-line | 使用 /statusline 设置自定义状态栏，将显示在输入框下方 |
| prompt-queue | 在 Claude 工作时按回车键可排队发送更多消息 |
| enter-to-steer-in-relatime | 在 Claude 工作时向其发送消息以实时引导 |
| todo-list | 在处理复杂任务时让 Claude 创建待办事项列表，以跟踪进度并保持正轨 |
| ide-upsell-external-terminal | 将 Claude 连接到你的 IDE · /ide |
| install-github-app | 运行 /install-github-app 即可在 GitHub Issue 和 PR 中 @提及 Claude |
| install-slack-app | 运行 /install-slack-app 以在 Slack 中使用 Claude |
| permissions | 使用 /permissions 预先批准和预先拒绝 bash、编辑和 MCP 工具 |
| drag-and-drop-images | 你知道吗，你可以将图片文件拖拽到终端中？ |
| paste-images-mac | 使用 control+v（不是 cmd+v！）将图片粘贴到 Claude Code 中 |
| double-esc | 双击 Esc 键将对话回退到之前的某个时间点 |
| double-esc-code-restore | 双击 Esc 键将代码和/或对话回退到之前的某个时间点 |
| continue | 运行 claude --continue 或 claude --resume 恢复对话 |
| rename-conversation | 使用 /rename 为对话命名，以便日后在 /resume 中轻松找到 |
| custom-commands | 通过向项目中的 .claude/skills/ 添加 .md 文件创建技能，或添加到 ~/.claude/skills/ 以创建在任何项目中都可用的技能 |
| custom-agents | 使用 /agents 优化特定任务，例如：软件架构师、代码编写者、代码审查者 |
| agent-flag | 使用 --agent <代理名称> 直接与子代理开始对话 |
| desktop-app | 使用 Claude 桌面应用在本地或远程运行 Claude Code：clau.de/desktop |
| web-app | 在云端运行任务，同时继续本地编码 · clau.de/web |
| voice-mode | 使用 /voice 启用按键通话听写功能 |
| no-flicker | 试试无闪烁渲染，现已支持鼠标操作 · /tui fullscreen |
| team-artifacts | 从团队发现状态中展示团队工件建议 |
| plan-mode-for-complex-tasks | 使用计划模式在进行更改前为复杂请求做好准备。按 <cycle-mode key> |
| terminal-setup | 运行 /terminal-setup 启用便捷的终端集成，如 Option+Enter 换行等 |
| shift-enter | 按 Option+Enter（Apple 终端）或 Shift+Enter 发送多行消息 |
| shift-enter-setup | 运行 /terminal-setup 启用 Option+Enter（Apple 终端）或 Shift+Enter 换行 |
| vscode-command-install | 打开命令面板（Cmd+Shift+P）并运行 "Shell Command: Install '<编辑器>' command in PATH" 以启用 IDE 集成 |
| shift-tab | 按 <cycle-mode key> 切换聊天模式 |
| image-paste | 使用 <image-paste key> 粘贴图片 |
| desktop-shortcut | 使用 <suggested shortcut> 在 Claude Code Desktop 中继续会话 |
| remote-control | 通过远程控制将会话与手机配对 |
| push-notif | 在长时间任务完成时收到手机通知 —— 在 <settings menu> 中启用推送通知 |
| opusplan-mode-reminder | 你的默认模型设置为 Opus 计划模式。按 <cycle-mode key> |
| frontend-design-plugin | 正在使用 HTML/CSS？安装前端设计插件 |
