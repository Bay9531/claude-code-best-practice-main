...
<p align="center">
  <img src="assets/impl-agent-teams.png" alt="代理团队运行中 — 带 tmux 的分屏模式" width="100%">
</p>

代理团队（Agent Teams）会生成**多个独立的 Claude Code 会话**，通过共享任务列表进行协调。与子代理（单个会话内的隔离上下文分支）不同，每个团队成员拥有自己完整的上下文窗口，CLAUDE.md、MCP 服务器和技能都会自动加载。

---

## ![如何使用](../!/tags/how-to-use.svg)

时间编排工作流完全由代理团队构建。运行最终成果：
...
这会调用**命令 → 代理 → 技能**流水线：代理获取迪拜的当前时间，技能将 SVG 时间卡片渲染到 `agent-teams/output/dubai-time.svg`。

---

## ![如何实现](../!/tags/how-to-implement.svg)

你可以使用代理团队创建天气编排工作流的副本——在本示例中，时间编排工作流完全由代理团队构建。
...
将以下提示词粘贴到 Claude 中，使用代理团队引导完成完整的时间编排器工作流：
