...
`/loop` 技能用于按 cron 间隔调度重复任务。以下是 `/loop 1m "tell current time"` 的演示——一个每分钟触发的简单重复任务。
...
- 重复任务 **3 天后自动过期**
- 任务具有**会话作用域**——仅存在于内存中，Claude 退出时停止
...
任务每分钟触发，运行 `date` 并报告当前时间。每次迭代触发异步的 **UserPromptSubmit** 和 **Stop** 钩子——与本仓库中用于声音通知的钩子系统相同。
...
`/loop` 是 Claude Code 的内置技能——无需任何设置。它在底层使用 cron 工具（`CronCreate`、`CronList`、`CronDelete`）来管理重复调度。
