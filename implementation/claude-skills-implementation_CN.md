...
本仓库中实现了两个技能，作为**命令 → 代理 → 技能**架构模式的一部分，演示了两种不同的技能调用模式：**代理技能**（预加载）和**技能**（直接调用）。
...
description: 从 Open-Meteo API 获取迪拜（阿联酋）当前天气温度数据的说明
...
这是一个**代理技能**——通过 `skills:` 前置元数据字段在启动时预加载到 `weather-agent` 中。它不被直接调用；而是作为注入代理上下文中的领域知识。注意 `user-invocable: false` 将其从 `/` 命令菜单中隐藏。
...
让 Claude 为你创建一个——它将生成带有 YAML 前置元数据和正文的 markdown 文件，位于 `.claude/skills/my-skill/SKILL.md`
