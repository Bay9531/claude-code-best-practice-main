...
天气代理在此仓库中实现，作为**命令 → 代理 → 技能**架构模式的示例，演示了两种不同的技能模式。
...
---
name: weather-agent
description: 当你需要获取迪拜（阿联酋）的天气数据时，主动使用此代理。
  此代理使用预加载的 weather-fetcher 技能从 Open-Meteo 获取实时温度。
allowedTools:
  - "Read"
  - "Skill"
model: sonnet
...
2. **报告**：向调用者返回温度值和单位
3. **记忆**：使用读取详情更新代理记忆以进行历史追踪

...
```

该代理有一个预加载的技能（`weather-fetcher`），提供从 Open-Meteo 获取数据的说明。它将温度值和单位返回给调用的命令。
...
或者让 Claude 为你创建一个——它将生成带有 YAML 前置元数据和正文的 markdown 文件，位于 `.claude/agents/<name>.md`
...
天气代理是命令 → 代理 → 技能编排模式中的**代理**。它从 `/weather-orchestrator` 命令接收工作流，并使用预加载的技能（`weather-fetcher`）获取温度。然后命令调用独立的 `weather-svg-creator` 技能来创建可视化输出。
